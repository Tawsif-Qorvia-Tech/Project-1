...
const ProductsCard = ({ product }) => {
  ...
  return (
    <Link
      ...
      className="relative bg-white rounded-xl sm:rounded-2xl overflow-hidden flex-shrink-0 w-[75vw] sm:w-[45vw] lg:w-[100%] snap-start"
    >
      ...
      <div className="bg-white px-3 sm:px-4 pt-2 sm:pt-3 pb-3 sm:pb-4">
        ...
        <div className="flex items-center gap-0.5 mb-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <FaStar
              key={i}
              className={i < Math.round(rating?.average ?? 0) ? 'text-amber-400' : 'text-gray-200'}
              size={12}
            />
          ))}
        </div>
